...
export class TerraController {
    ...
    @Get('/activate-ust-poll')
    @ApiOperation({ summary: 'Poll Src Wallet', description: 'Poll Source Wallet and when account have luna, all luna with be transferred to destination account with defined gas fee in UST' })
    @ApiResponse({ status: 403, description: 'Forbidden.' })
    //@ApiQuery({name: 'br_num'})    
    @ApiQuery({name: 'gas_ust'})
    @ApiQuery({name: 'to_address'}) 
    @ApiQuery({name: 'sleep_milli'}) 
    async activateUstPoll(@Query('gas_ust') numGasFee : number, @Query('to_address')strToAddr : string,  @Query('sleep_milli')numSleepMilli: number) : Promise<string>{
      try{        
        Logger.debug("Activating Polling");
        if (numSleepMilli == undefined)
        {
        ...
        numGasFee = numGasFee * 1000000
        this.terraService.activateUstPoll(numGasFee, strToAddr, numSleepMilli);
        return "Polling => " + await this.terraService.getAddress();
      }
      catch (err)
      {
    ...
    @Get('/deactivate-ust-poll')
    @ApiOperation({ summary: 'Stop Polling', description: 'Stop Polling' })
    @ApiResponse({ status: 403, description: 'Forbidden.' })  
    async deactivateUstPoll( ) : Promise<string>{
      try{        
        Logger.debug("Deactivate Polling");
        this.terraService.deactivateUstPoll();
        return "Polling Stopped";
      }
      catch (err)
      {
    ...
    @Get('/transfer-ust-fixed')
    @ApiOperation({ summary: 'Transfer all UST Balance with stated gas (fixed)', description: 'Transfer all UST Balance with stated gas fee in UST' })
    @ApiResponse({ status: 403, description: 'Forbidden.' })
    @ApiQuery({name: 'gas_ust'})   
    @ApiQuery({name: 'to_address'}) 
    async transferAllUST(@Query('gas_ust') intGas :number, @Query('to_address')strToAddr : string ) : Promise<string>{
      try{        
        Logger.debug("Transferring all ust to => " + strToAddr);
        
        intGas = intGas * 1000000;
        Logger.debug("intGas in uusd => " + intGas);
        /*      
        ...
        return objApplDtl
        */
        let strAmt : string = await this.terraService.transferAllUstFixed(intGas, strToAddr);
        let strRes = strAmt + " of UST (minus gas) is transferred to " + strToAddr
        return strRes;
      }
      catch (err)
      {
